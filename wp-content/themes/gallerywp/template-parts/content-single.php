                ...
                if ( !(gallerywp_get_option('hide_thumbnail')) ) {
                    if ( !(gallerywp_get_option('hide_thumbnail_single')) ) {
                        if ( gallerywp_get_option('thumbnail_link') == 'no' ) {
                            the_post_thumbnail('gallerywp-featured-image', array('class' => 'gallerywp-post-thumbnail-single'));
                        } else { ?>
                            <a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'gallerywp' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_post_thumbnail('gallerywp-featured-image', array('class' => 'gallerywp-post-thumbnail-single')); ?></a>
                <?php   }
                    }
                }
            }
